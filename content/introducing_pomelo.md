...
Being in a time of my life where i had quite a lot of spare time, writing a homemenu for a console i know nothing about sounded like quite a cool project, that i really fell in love with over time.
...
## Installation
Pomelo is using Luma3ds "Game Patching" to replace the stock nintendo homemenu, note that "Game Patching" is an optional feature, that has to be enabled. Currently i only tested it on my console which is a New 3DS US.
...
## Testing
Pomelo is using Luma3ds Game Patching to load itself, instead of the stock homemenu, in case something goes wrong, simply remove the Pomelo `code.bin` and `exheader.bin` files from the SDCard and the stock homemenu should boot as normal.
However POMELO HAS NOT BEEN THOROUGHLY TESTED, **USE AT YOUR OWN RISK**.

## Screenshots!
Pomelo running on real New 3DS hardware:

![Pomelo running on a New 3DS console, showing the game grid](/images/introducing_pomelo/screenshot_09-Jul-2026_14-11-00.png)

Pomelo running in Mikage emulator:

![Pomelo running in the Mikage emulator, showing the game grid](/images/introducing_pomelo/screenshot_12-Jun-2026_13-55-28.png)
