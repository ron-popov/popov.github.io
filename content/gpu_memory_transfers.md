...
In the previous 3 blogs, i was talking about a low level issue. When a 3ds process starts, it requests from the kernel a memory region it can use as heap, and one it can use as linear heap - heap for storing textures and GPU stuff.
The unique part about linear heap, is that it's continous in the physical memory as well. This is very important when storing buffers that the GPU will access, as the GPU doesn't have an MMU and works mostly with physical addresses.

For regular apps, libctru (open source 3ds SDK) just takes care of it, but when you are building a homemenu, it's not that simple...
...
The 3ds has the PICA200 GPU that we will be talking about today. And a sysmodule called the `GSP` module that acts as a driver for communicating with the GPU.
We also have `citro3d`, which is a usermode sdk, meaning the it's part of the app that wants to render stuff, that we will be using as an abstraction layer.

Assuming we have already initialized everything required for rendering, let's look at what happens every frame we render :)

## Frame Init
One of the most important parts of the rendering process is the `gpuCmdBuffer`, it contains all the render instructions for the GPU, including textures to render, text, vectors (lines, rectangles). It also includes the physical addresses that point to the location of these textures, vectors in physical memory. Again, the GPU doesn't really know what virtaul memory is.
...
After pomelo added all the textures and everything it wants to render, we have a `gpuCmdBuffer` that is filled with instructions for the GPU to process and make the magic happen.
...
This call is the most complicated by far, so i want to focus on it a bit more. Also (spoiler alert), this is the call that caused pomelo to hang, we will talk more about it very soon.

It tells the GSP module to render the beautiful `gpuCmdBuffer` that we have built.

We give the **GSP module** the Virtual Address of the `gpuCmdBuffer` and it's size. The GSP module translates this virtual address into a physical address, that the GPU can actually use and access. 

Unlike other common operating systems, on the 3ds translating the virtual address to a physical address is fairly easy, in most cases it's simply a hardcoded offset between the two addresses.

Then the GSP module is giving the GPU the physical address of the `gpuCmdBuffer` and it's size, and the GPU does it's magic.

### 3. DisplayTransfer
This call tells the GSP module to tell the GPU to start rendering all the processing it just did.
...
Each such GSP call has a signal of it's own that indicates the action finished:
...
After we transfered Pomelo to use the System region in FCRAM, instead of the APP region, which you can read more about in the previous blogs.
I suddenly noticed that pomelo started hanging shortly after booting...
...
So i continued debugging, and the issue was that we were waiting for a signal that the GPU never sent, but which signal is the missing one?

I printed all the signals and stubbed each of the calls to the GSP module, to identify the GSP call that was causing the hang, and i learned that the **culprit was `ProcessCommandList`**.
...
There were some changes to the address of VADDR in kernel version 2.44 of the 3ds kernel, and that was my first suspect.
...
Because this is a sysmodule and not a regular title, we can't use "Game Patching" like we did in the previous blog, we need to use "sysmodule patching", that works by giving Luma3ds a patch file to patch the real GSP module.
...
As you can see in the crash dumps, the physical address of the buffer that pomelo allocated is `0x280BBC00`. The physical address of the stock homemenu buffer is `0x28005AA0`.
...
I was trying to get claude to fix the hanging GPU issue, not with much success :(

But it did raise an interesting point. He saw i am using the stock font that comes with the console. Claude was thinking that the font was in some shared memory region that could be unreachable to the GPU.
...
Maybe one of them tries to access some weird memory region and this hangs the GPU? Maybe one of the textures / vectors / whatever is passed to the GPU, is mapped in some weird memory region, that the GPU cannot access?

There is only one way to find out!

So we dumped the commands from the `gpuCmdBuffer` by going over the buffer and parsing the commands in the buffer. In practice, they are all GPU Register Writes, as this is how most of the interaction between the GPU and the CPU works, by writing to GPU registers.
...
This register is used as a base address for passing addresses of vertex buffer. When we want to point the GPU at the addr of a certain vertex buffer, we only need to give the GPU the offset from the base address, and that way we can save a couple of bytes :)

When the GPU renders vertices (triangles and such, NOT TEXTURES), pomelo is expected to store all the data about the vertices in a buffer in it's own memory, and give the GPU the address of that buffer, which should be allocated in the linear heap. Access to the vertices buffers works by giving the GPU a base address + offset.

But that is an important BUT - **the GPU can only access the first 256MB from the base address**, meaning the physical addresses that can be accesses are, base address -> base address + 256MB.
...
```
(FCRAM System Region Start) - (Citro3d base address) = 0x28000000 - 0x18000000 = 0x10000000 = Exactly 256MB!
```
